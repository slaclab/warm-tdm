# software/examples/cpp_extension — dead prototype, kept as a C++-module skeleton

**Unmaintained reference; no build or runtime support is claimed.**
...
glue are the tedious parts to reconstruct, so we retain the source example rather than
rederive them.
...
```
software/examples/cpp_extension/
├── CMakeLists.txt          # standalone project; finds rogue/boost, sets the
│                           #   .so name (PREFIX "" SUFFIX ".so") and points
│                           #   LIBRARY_OUTPUT_DIRECTORY to <build>/python; add
│                           #   that path explicitly when adapting the example
...
```bash
cd software/scripts && python warmTdmServer.py --emulate
```
