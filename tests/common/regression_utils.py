...
def run_warm_tdm_vhdl_test(
    *,
    ...
    # Simulator selection: default to GHDL (license-free, fast) for benches that
    # contain no vendor IP. Benches whose DUT instantiates Xilinx IP (e.g. the
    # AdcDspFp FP cores) must run under VCS/XSIM, which can model the IP given
    # the Vivado compiled sim libraries. Override via WARM_TDM_SIM or the
    # `simulator=` argument.
    simulator = os.environ.get("WARM_TDM_SIM", simulator) or "ghdl"
    ...
    run(
        ...
        simulator=simulator,
        vhdl_compile_args=COMMON_VHDL_COMPILE_ARGS if simulator == "ghdl" else None,
    )
