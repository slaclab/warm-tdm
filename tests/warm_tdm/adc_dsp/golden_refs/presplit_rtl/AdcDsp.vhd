...
-- SNAPSHOT-LOCAL ADDITION (not in the original 5645f7e source): the pre-split
-- source left numeric_std commented out and relied on the synopsys
-- std_logic_unsigned package (in scope in its original build) for the slv
-- `pidStateRamAddr + 1` counter increment. Restore it explicitly so the golden
-- reference elaborates under today's GHDL; unsigned increment semantics are the
-- faithful, behavior-preserving reading for a RAM-address counter. See
...
--                v.state         := PID_P_S;
   ...
   U_AxiStreamFifoV2_PID_DEBUG : entity surf.AxiStreamFifoV2
      generic map (
         ...
         FIFO_ADDR_WIDTH_G   => 9,
         SYNTH_MODE_G        => "inferred",  -- SNAPSHOT-LOCAL: was "xpm" (see README.md)
         MEMORY_TYPE_G       => "bram",
