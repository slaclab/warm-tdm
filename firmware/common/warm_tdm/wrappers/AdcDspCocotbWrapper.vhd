-------------------------------------------------------------------------------
-- Title      : AdcDsp cocotb wrapper (fixed-point PID)
-------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------
-- Description:
-- Thin cocotb-facing wrapper for the fixed-point AdcDsp entity.
--
-- The fp-pid AdcDsp interface no longer consumes a raw ADC AXI-stream. The ADC
-- accumulation now lives in the separate AdcAccumulator entity, and AdcDsp
-- receives a decoded `accumIn : AdcAccumResultType` qualified by `accumValid`
-- plus a decoded `timingRxData : LocalTimingType`. This wrapper flattens those
-- records into scalar cocotb-drivable ports so the Python bench can present a
-- single accumulation result (accumError / rowIndex / seqStart / ...) and pulse
-- accumValid, rather than feeding raw ADC samples.
--
-- The DUT is instantiated with SIMULATION_G => true so its AXI-stream FIFOs use
-- inferred logic that GHDL can elaborate. The mAxil DAC-write port is looped
-- into a local AxiDualPortRam sink so the internal AxiLiteMaster always sees a
-- responding slave.
-------------------------------------------------------------------------------
...
begin
   ...
   ----------------------------------------------------------------------------
   -- DUT hookup (new accumIn / accumValid interface)
   ----------------------------------------------------------------------------
