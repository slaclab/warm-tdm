...
| `sess.fas_tune(**params)` | Physical-line FAS tuning (`FasTuneProcess`); temporarily applies `Sq1BiasCurrent` (40 uA by default) to enabled columns. `FasFluxSampleReads` and `ServoSampleReads` discard post-write ADC reads so VCS time advances; pass `SetAfterFinish=True` to program the fitted `FasOn` currents. |
