...
Owner: [#108](https://github.com/slaclab/warm-tdm/issues/108). This is a separate
analyzer proposal, not acceptance for channelization or the PID firmware.

**Proposal baseline:** the text below preserves the original design discussion.
Its branch-reconciliation prerequisites and decoder defects have since been
addressed; integer debug is now v3 with a signed nine-bit net count. The statement
that an eight-bit count is adequate is superseded. Current source uses the
16-byte tagged header, a 96-byte integer frame and a 56-byte FP frame. Reconcile
all proposed telemetry/scaling with those contracts before implementation.
The metrics/diagnosis/recommendation/comparison API is not implemented at
`fae7151`. Ongoing scope and acceptance belong to #108.
